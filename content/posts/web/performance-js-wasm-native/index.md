...
{{< faq summary="Wasm과 Native의 성능 차이가 없다고 보면 되나요?" >}}
- 위의 예제는 Wasm 함수를 돌리기 위해 정수 인자 하나만 전달하면 되는 형태로 `JS 접착 코드` ↔ `Wasm` 상호작용이 거의 없어 Wasm의 성능을 극대화할 수 있는 형태입니다.
- 다른 문제에서 `사용자` ↔ `JS 접착 코드` ↔ `Wasm` 상호작용이 자주 발생하는 경우, 경계를 넘는 호출 자체의 오버헤드가 쌓입니다. 특히 문자열이나 배열처럼 큰 데이터를 주고받을 때는 해당 데이터를 Wasm의 선형 메모리로 복사해 넣고 결과를 다시 꺼내는 과정이 필요해, 그 비용 때문에 Native만큼의 성능이 나오지 않을 수 있습니다. 이 과정에 대한 자세한 설명은 [JavaScript에서 C++ 함수 호출 하기 - ccall, cwrap](/posts/call-cpp-from-js/) 참고
- 따라서 Wasm이 동작할 때는 JS 접착 코드와의 상호작용을 최소화하도록 설계하는 것이 성능을 높일 수 있습니다.
{{< /faq >}}

{{< faq summary="왜 JavaScript가 1.5배밖에 안 느린가요?" >}}
...
{{< faq summary="어떤 작업에 Wasm이 유리한가요?" >}}
